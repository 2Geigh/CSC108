...
'Montreal Canadiens': [1, 2, 1, 0, 2]}
    ...
    for line in game_data:
        if line[0].isalpha():
            current_team = line[:line.index('\n')]
            if current_team not in return_dict:
                return_dict[current_team] = []
        else:
            return_dict[current_team].append(int(line[:line.index('\n')]))
        
    return return_dict

input_file = open('sample_games.txt')
print(get_game_dict(input_file) == {'Toronto Maple Leafs': [2, 2, 1, 0, 0, 2], 'Grande Prairie Storm': [], \
'Montreal Canadiens': [1, 2, 1, 0, 2]})